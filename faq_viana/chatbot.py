...
def print_welcome(language, bot_name):
    if language == "pt":
        ...
        print(f"Hello! You are chatting with the assistant '{bot_name}'.")
...
if __name__ == "__main__":
    ...
    current_language = "pt"
    category_keywords, cat_to_questions, cat_to_answers = load_qas_and_categories(bot_path, current_language)
    neg_path = os.path.join(bot_path, f"negative_feedback_{current_language}.txt")
    neg_feedback = load_negative_feedback(neg_path)

    print_welcome(current_language, selected_bot)
